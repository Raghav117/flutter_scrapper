...
class CacheManager {
  ...
  CacheStats getStats() {
    ...
    return CacheStats(
      ...
      oldestEntry: validEntries.isNotEmpty 
          ? validEntries.map((e) => e.cachedAt).reduce((a, b) => a.isBefore(b) ? a : b)
          : null,
      newestEntry: validEntries.isNotEmpty
          ? validEntries.map((e) => e.cachedAt).reduce((a, b) => a.isAfter(b) ? a : b)
          : null,
    );
  }
  ...
  Future<void> _enforceLimits() async {
    ...
    if (_memoryCache.length > _config.maxEntries) {
      ...
      final toRemove = sortedEntries.take(_memoryCache.length - _config.maxEntries);
